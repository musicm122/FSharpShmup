...
type AnxietyFs() =
    ...
    member this.SlowAudio = this.GetNode(new NodePath("SlowAudio")) :?> AudioStreamPlayer

    member val IsAggro = false with get, set

    member this.AreaEffect: Area2D = this.GetNode<Area2D>(new NodePath("Slow"))

    member this.RemoveSlowFromPlayer(player: PlayerFs) = player.Speed <- player.DefaultSpeed
    ...
    override this._Ready() =
        ...
        this.HpProvider.OnDeath <-
            this.RemoveSlow(this.GetPlayer())
            this.DieAudio.Play()
            this.QueueFree


    member this.GetSqDistanceBetweenSelfAndTarget() = this.GetPlayer().GlobalPosition.DistanceTo(this.GlobalPosition)
    ...
    member this.OnSlowBodyExited(body: Node) = this.RemoveSlow body

    member this.OnSlowBodyEntered(body: Node) = this.ApplySlow body
    ...
    member this.ApplySlow(body: Node) =
        ...
        | "player" ->
            ...
            ship.Speed <- ship.DefaultSpeed - 25f
            this.IsAggro <- true
        | _ -> ignore()
